"""
Module 9: Cleanup & Cost Optimization Page

This page provides a dedicated interface for the Cleanup & Cost Optimization module,
using the CleanupCollector and CleanupAnalyzer classes.
"""
...
from collectors.mod9_cleanup_collector import CleanupCollector
from analyzers.mod9_cleanup_analyzer import CleanupAnalyzer
...
def initialize_session_state():
    ...
    if 'mod9_collected_data' not in st.session_state:
        st.session_state.mod9_collected_data = None
    if 'mod9_analyzed_data' not in st.session_state:
        st.session_state.mod9_analyzed_data = None
    if 'mod9_findings' not in st.session_state:
        st.session_state.mod9_findings = None


def display_kpi_cards(analyzed_data: dict):
    """Display KPI cards for all 2 components."""
    st.subheader("KPI Cards - Cleanup & Cost Optimization")
    
    # Extract component counts
    empty_tables_df = analyzed_data.get('01_empty_tables', pd.DataFrame())
    stale_temp_df = analyzed_data.get('02_stale_temp_tables', pd.DataFrame())
    
    empty_tables_count = len(empty_tables_df)
    stale_temp_count = len(stale_temp_df)
    
    # Calculate severity counts
    severity_counts = {'CRITICAL': 0, 'HIGH': 0, 'MEDIUM': 0, 'LOW': 0, 'INFO': 0}
    total_findings = 0
    
    for component_key in ['01_empty_tables', '02_stale_temp_tables']:
    ...
    with col1:
        st.metric("Tablas Vacías", empty_tables_count)
    with col2:
        st.metric("Tablas Temporales Antiguas", stale_temp_count)
    with col3:
...
def display_findings_table(analyzed_data: dict):
    ...
    component_names = {
        "01_empty_tables": "Empty Tables",
        "02_stale_temp_tables": "Stale Temp Tables"
    }
    ...
    st.download_button(
        label="Descargar CSV",
        data=csv,
        file_name=f"cleanup_findings_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv"
    )
...
def display_ddl_actions(analyzed_data: dict):
    ...
    if all_ddl:
        ...
        st.download_button(
            ...
            data=combined_ddl,
            file_name=f"cleanup_ddl_actions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.sql",
            mime="text/plain"
        )


def display_empty_tables_details(analyzed_data: dict):
    """Display empty tables details."""
    st.subheader("Detalles de Tablas Vacías")
    ...
    if not empty_tables_df.empty:
        ...
        st.info("No hay tablas vacías disponibles")


def display_stale_temp_details(analyzed_data: dict):
    """Display stale temp tables details."""
    st.subheader("Detalles de Tablas Temporales Antiguas")
    
    stale_temp_df = analyzed_data.get('02_stale_temp_tables', pd.DataFrame())
    
    if not stale_temp_df.empty:
        st.dataframe(
            stale_temp_df,
            width='stretch',
            height=300
        ...
        st.info("No hay tablas temporales antiguas disponibles")
...
def main():
    ...
    st.title("Cleanup & Cost Optimization")
    st.markdown("*Identificación de Basura Digital y Ahorro de Espacio en Base de Datos*")
    st.markdown("---")
    ...
    if st.sidebar.button("Ejecutar Análisis Módulo 9", type="primary"):
        try:
            ...
            # Step 2: Collect data using CleanupCollector
            with st.spinner("Recolectando datos de limpieza..."):
                ...
                st.session_state.mod9_collected_data = collected_data
            ...
            # Step 3: Analyze data using CleanupAnalyzer
            with st.spinner("Analizando datos..."):
                analyzer = CleanupAnalyzer()
                analyzed_data = analyzer.run(collected_data)
                st.session_state.mod9_analyzed_data = analyzed_data
                st.session_state.mod9_findings = analyzer.get_findings()
                
                logger.info(f"Analysis complete. Total findings: {len(st.session_state.mod9_findings)}")
            
            connection.close()
            st.success(f"Análisis completado. Total hallazgos: {len(st.session_state.mod9_findings)}")
    ...
    if st.session_state.mod9_analyzed_data:
        st.markdown("## Resultados del Análisis")
        
        display_kpi_cards(st.session_state.mod9_analyzed_data)
        st.markdown("---")
        display_empty_tables_details(st.session_state.mod9_analyzed_data)
        st.markdown("---")
        display_stale_temp_details(st.session_state.mod9_analyzed_data)
        st.markdown("---")
        display_findings_table(st.session_state.mod9_analyzed_data)
        st.markdown("---")
        display_ddl_actions(st.session_state.mod9_analyzed_data)
